...
if (isset($_POST['submit'])) {
    ...
    $sqlins = "UPDATE `actividadcatalogo` 
    SET `actividad`='$actividad',`descripcion`='$descripcion' WHERE id='$id'";
    $result = mysqli_query($conn, $sqlins);

    if ($result) {
        header("location: actividadesCatalogos.php?msg=Actividad actualizado con éxito.");
    }
}
?>
...
<body>
   ...
   <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
    ...
    <div class="container" style="margin-bottom: 35px;">
        <div class="text-center mb-4 fs-4">Editar Actividad</div>
        <?php
        $id = $_GET['id'];
        $sql = "SELECT * FROM `actividadcatalogo` WHERE id = '$id' LIMIT 1";
